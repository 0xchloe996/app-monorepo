...
### Hooks Safety
- **useEffect with missing dependencies**: eslint-disable hiding dependency issues
- **Non-functional setState in useCallback**: Using stale state closures
- **Missing cleanup in useEffect**: Timers, subscriptions, abort controllers not cleaned up
- **Async validation without abort**: No cancellation on unmount

### Re-render Performance
- **Inline objects/functions in JSX**: Creating new references every render
- **Missing memoization**: Heavy computations without useMemo
- **State placed too high**: Causing wide re-render fanout
- **Unstable callback references**: Missing useCallback for event handlers passed to children
