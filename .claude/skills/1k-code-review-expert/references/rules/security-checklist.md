...
## OneKey-Specific Security

- **Wallet secrets**: Never expose mnemonics, private keys, or signing payloads in logs/errors/analytics
- **Extension permissions**: Check manifest.json for overly broad permissions
- **WebView security**: Verify no sensitive data leakage through WebView bridges
- **IPC security**: Validate all Electron IPC messages, never trust renderer process
